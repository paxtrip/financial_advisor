...
- **Система хештегов** — свободные метки для транзакций, позиций чека и магазинов. Можно писать прямо в тексте (`кофе 250р #работа`), LLM предлагает теги автоматически, теги магазинов наследуются новыми транзакциями
- **Фильтрация по тегам** — `/report #дача` покажет все расходы с этим тегом; после обычного отчёта появляются кнопки для быстрой фильтрации
- **Редактирование** — изменение и удаление записей через inline-кнопки; добавление тегов после сохранения
...
│   └── tag_parser.py        # Извлечение и нормализация хештегов
...
Категории иерархические — `categories.parent_id` связывает подкатегории с родительскими. Теги свободные — `tags` хранит справочник, связи через `transaction_tags` / `transaction_item_tags` / `store_tags`. Подробная схема — в файле [`supabase_schema.sql`](supabase_schema.sql).
...
- Постоянные теги магазинов через диалог (`/store_tag Пятёрочка #продукты`)
- Голосовой ввод
- Экспорт в CSV
